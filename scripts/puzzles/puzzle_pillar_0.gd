...
func _process(delta):
	Globals.puzzle_0_active = this_puzzle_active
	Globals.puzzle_0_viewport = node_viewport.gui_disable_input
...
func look():
	Globals.playernode.queue_message("Some sort of puzzle lock. This one has been solved already.", 4, load("res://assets/VO/puzzle_solved_already.wav"))
...
func _mouse_entered_area():
	print("puzzle_pillar_0.gd: Mouse entered callback")
	Globals.puzzle_pillar_0_mouse_entered = true
	is_mouse_inside = true

func _mouse_exited_area():
	print("puzzle_pillar_0.gd: Mouse exited callback")
	Globals.puzzle_pillar_0_mouse_entered = false
	is_mouse_inside = false

func _unhandled_input(ev):
	print("puzzle_pillar_0.gd: Unhandled Input Called")
	if this_puzzle_active:
		print("puzzle_pillar_0.gd: Unhalnded input proceeding because puzzle active")
		print("puzzle_pillar_0.gd: Raw ev - " + str(ev))
		var is_mouse_event = false
		for mouse_event in [InputEventMouseButton, InputEventMouseMotion, InputEventScreenDrag, InputEventScreenTouch]:
			if ev is mouse_event:
				print("puzzle_pillar_0.gd: Mouse event detected. Breaking to forward to viewport")
				is_mouse_event = true
				break
		#if is_mouse_event and (is_mouse_inside or is_mouse_held):
		if is_mouse_event:
			print("puzzle_pillar_0.gd: Pass mouse event to viewport")
			Globals.puzzle_pillar_mouse_poisition = ev.position
			Globals.puzzle_pillar_mouse_global_poisition = ev.global_position
			handle_mouse(ev)
